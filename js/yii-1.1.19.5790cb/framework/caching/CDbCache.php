...
EOD;
	...
	protected function flushValues()
	{
        ...
        /**
         * Gets the expiration value for a given cache id.
         * @param string $key the key of the cached item to be checked
         * @return integer the expiration of the cache item
         */
        public function expiration($key)
        {
            $hashedKey = $this->generateUniqueKey($key);
            $sql="SELECT expire FROM {$this->cacheTableName} WHERE id='$hashedKey'";
            $command=$this->getDbConnection()->createCommand($sql);
