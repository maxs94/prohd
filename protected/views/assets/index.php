...
if(!($assetInterface->getCacheExpiration($groupMembers[0]->characterID)))
{
	...
	foreach ($groupMembers as $member)
	{
		...
		//$this->storeAssetValues(Yii::app()->user->trackingGroupID);
	}
}
...
$(document).ready(function()  {
	...
	$('#asset-refresh').click(function(){
		...
		$.getJSON('./index.php?r=assets/getAssetsJson', function(data) {
...
<h1>Assets - <?php echo $group->name; ?></h1>
...
<table id="assets" style="font-size: 12px">
	...
	<?php
		...
		<?php
			...
			$baseAssets = Assets::Model()->with('values')->findAll($criteria);
			...
			<?php
				...
				$contents = Assets::Model()->with('values')->findAll('containerID=:containerID AND characterID IN '.$groupMembersString.'',array(':containerID'=>$baseAsset->itemID));
				if ($contents)
				{
					$idx = 1;
